...
int main()
{
	...
	*/
	...
	struct flex
	{
		size_t count;
		double average;
		double values[];	// flexible array member (last mamber!)
	};
	...
	printf("Addrsss of pf->values %lld\n", (long long)&pf->values);
	...
	struct nonflex nf;
	nf.value = (double*)malloc(sizeof(double) * n);	//구조체가 저장된 공간과 다른곳에 저장됨.
	*/
	...
	*pf2 = *pf1;		// Don't copy flexible members, use memcopy() instead
